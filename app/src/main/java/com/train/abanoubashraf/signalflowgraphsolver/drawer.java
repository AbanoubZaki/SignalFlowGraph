...
public class drawer extends View {
    ...
    private void startDrawing(Canvas canvas) {
        ...
        int numOfNodes = SFG.getInstance().getGraph().getVertexes().size();
        double[][] gains = SFG.getInstance().getGraph().adjMatrix;
        ...
        for (int i = 0; i < numOfNodes; i++)
            canvas.drawCircle(xDisplace * (i + 1) - nodeRadius + 25, yCenter
                    - nodeRadius + 25 , nodeRadius, paint);

        //write nodes names.
        paint.setColor(Color.BLACK);
        for (int i = 0; i < numOfNodes; i++)
            canvas.drawText(SFG.getInstance().getGraph().getVertexes().get(i).label, xDisplace * (i + 1) - nodeRadius + 20, yCenter + 4, paint);
